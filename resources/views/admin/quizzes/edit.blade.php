...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-slate-900">Kelola Soal Kuis</h2>
            <p class="text-xs text-slate-500">Kelola kuis dan daftar pertanyaan</p>
        </div>
    ...
    @php
        $isCustom = empty($quiz->heritage_slug);
    @endphp

    <form action="{{ route('admin.quizzes.update', $quiz->id) }}" method="POST" class="bg-white rounded-3xl p-8 shadow-sm border border-amber-900/10 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label class="p-4 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-500 transition flex items-center space-x-3 bg-slate-50">
                    <input type="radio" name="quiz_mode" value="heritage" {{ !$isCustom ? 'checked' : '' }} onchange="toggleQuizMode(this.value)" class="text-amber-600 focus:ring-amber-500">
                    <div>
                ...
                <label class="p-4 rounded-2xl border border-slate-200 cursor-pointer hover:border-amber-500 transition flex items-center space-x-3 bg-slate-50">
                    <input type="radio" name="quiz_mode" value="custom" {{ $isCustom ? 'checked' : '' }} onchange="toggleQuizMode(this.value)" class="text-amber-600 focus:ring-amber-500">
                    <div>
        ...
        <!-- Heritage Section -->
        <div id="section-heritage" class="{{ $isCustom ? 'hidden' : '' }}">
            ...
            <select name="heritage_slug" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 font-semibold text-slate-800">
                @foreach($heritages as $h)
                    <option value="{{ $h->slug }}" {{ $quiz->heritage_slug === $h->slug ? 'selected' : '' }}>{{ $h->name }} ({{ $h->category_name }})</option>
                @endforeach
            </select>
        </div>

        <!-- Custom Section -->
        <div id="section-custom" class="{{ !$isCustom ? 'hidden' : '' }} space-y-4">
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Judul Kuis Custom</label>
                <input type="text" name="title" value="{{ $quiz->title }}" placeholder="Contoh: Tantangan Mahakarya Kerajaan & Candi" class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Deskripsi Kuis</label>
                <textarea name="description" rows="2" placeholder="Jelaskan mengenai tantangan kuis ini..." class="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">{{ $quiz->description }}</textarea>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                    @foreach($categories as $c)
                        <label class="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl flex items-center space-x-2 text-xs font-semibold cursor-pointer">
                            <input type="checkbox" name="categories[]" value="{{ $c->name }}" {{ str_contains($quiz->category, $c->name) ? 'checked' : '' }} class="rounded text-amber-600 focus:ring-amber-500">
                            <span>{{ $c->name }}</span>
                        </label>
                    @endforeach
                </div>
        ...
        <div class="border-t border-slate-200 pt-6 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500">Tambahkan atau perbarui pertanyaan kuis, pilihan jawaban, dan fakta</p>
                </div>
            ...
            <div id="questions-container" class="space-y-4">
                @foreach($quiz->questions as $index => $q)
                    <div class="question-row p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 relative">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 mb-1">Pertanyaan #{{ $index + 1 }}</label>
                            <input type="text" name="questions[{{ $index }}][question]" value="{{ $q->question }}" required placeholder="Pertanyaan kuis..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-amber-500 bg-white">
                        </div>

                        <div class="grid grid-cols-2 gap-3">
                            <input type="text" name="questions[{{ $index }}][options][0]" value="{{ $q->options[0] ?? '' }}" required placeholder="Pilihan A" class="px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                            <input type="text" name="questions[{{ $index }}][options][1]" value="{{ $q->options[1] ?? '' }}" required placeholder="Pilihan B" class="px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                            <input type="text" name="questions[{{ $index }}][options][2]" value="{{ $q->options[2] ?? '' }}" required placeholder="Pilihan C" class="px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                            <input type="text" name="questions[{{ $index }}][options][3]" value="{{ $q->options[3] ?? '' }}" required placeholder="Pilihan D" class="px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-600 mb-1">Pilihan Jawaban Benar</label>
                                <select name="questions[{{ $index }}][correct_index]" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                                    <option value="0" {{ $q->correct_index == 0 ? 'selected' : '' }}>Pilihan A (Index 0)</option>
                                    <option value="1" {{ $q->correct_index == 1 ? 'selected' : '' }}>Pilihan B (Index 1)</option>
                                    <option value="2" {{ $q->correct_index == 2 ? 'selected' : '' }}>Pilihan C (Index 2)</option>
                                    <option value="3" {{ $q->correct_index == 3 ? 'selected' : '' }}>Pilihan D (Index 3)</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-600 mb-1">Penjelasan Jawaban</label>
                                <input type="text" name="questions[{{ $index }}][explanation]" value="{{ $q->explanation }}" placeholder="Penjelasan fakta..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs bg-white">
                            </div>
                        </div>
                    </div>
                @endforeach
